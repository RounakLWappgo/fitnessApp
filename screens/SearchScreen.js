...
export default SearchScreen = () => {
  return <ScrollView style={{marginTop:60}} showsVerticalScrollIndicator={false}>
      <Header name='Search' Bell={false} Search={true} Menu={true} arrow={true} leftBar={false}/>
      ...
      <Ionicon name="ios-search" size={20} color="#000"/>
          <TextInput style={{width:"95%", backgroundColor:"#F2F2F4", paddingHorizontal:10,paddingVertical:10, borderRadius:20}} placeholder='Search' />
      </View>
      <Text style={{fontSize:30, fontWeight:"bold", marginLeft:20, marginBottom:20, marginTop:30}}>Popular Courses</Text>

      <List horizontal={true}/>

      <Text style={{fontSize:30, fontWeight:"bold", marginLeft:20, marginBottom:20, marginTop:30}}>Top Trainers</Text>
      <View >
      {data.map((item, index) => {
          return <View key={index} style={{marginHorizontal:20, marginVertical:20, flexDirection:"row", alignItems:"center"}}>
                  ...
                  <View style={{marginLeft:"auto"}}>
                      <Text style={{fontWeight:"bold"}}>{item.followers}</Text>
